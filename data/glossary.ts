...
Gemini 1.5 Pro（2024年2月発表）では最大100万トークンのコンテキストウィンドウを実現し、長編文書・動画全体・大規模コードベースを一度に処理できるようになりました。Gemini 1.5 Flashは高速・低コストモデルとして、Gemini 2.0では強化されたreasoning能力とマルチモーダル出力（画像・音声生成）を実装。Google AI Studio・Vertex AI経由でAPIも提供されており、日本語性能も継続的に強化されています。`,
    relatedSlugs: ["llm", "multimodal", "vision-language-model", "foundation-model", "reasoning-model", "gemini-3"],
    sources: [
      {
        title: "Gemini: A Family of Highly Capable Multimodal Models",
        url: "https://arxiv.org/abs/2312.11805",
  ...
  },
  {
    ...
    description: `Gemini 3（ジェミニ スリー）は、Google DeepMindが2025年11月18日にリリースした第3世代マルチモーダルLLMシリーズです。

## Gemini 3 シリーズの構成

| モデル | リリース日 | 特徴 |
|--------|-----------|------|
| Gemini 3 Pro | 2025年11月18日 | 最高性能・マルチモーダル推論 |
| Gemini 3 Flash | 2025年11月 | 高速・低コスト |
| Gemini 3.1 Pro | 2026年2月19日 | Gemini 3 Proの次世代版、さらに高精度 |
| Gemini 3.1 Flash Lite | 2026年3月3日 | 軽量・開発者向け |

## ベンチマーク（Gemini 3.1 Pro）

Gemini 3.1 Proはベンチマークで特に高い評価を受けています。
- **ARC-AGI-2**: 77.1%（Gemini 3 Proから2倍以上に向上）
- **GPQA Diamond**: 94.3%（大学院レベルの科学問題）
- **SWE-Bench Verified**: 80.6%（実世界のソフトウェアエンジニアリング）

## 利用プラン（2026年現在）

2025年5月に「Gemini Advanced」から「Google AI Pro」に改名されました。

| プラン | 料金 | 主な特徴 |
|--------|------|---------|
| 無料 | 0円 | Gemini 2.0 Flash相当 |
| Google AI Plus | 低価格帯 | 基本的なGemini 3アクセス |
| Google AI Pro | ¥2,900/月 | Gemini 3.1 Proフルアクセス |
| Google AI Ultra | ¥30,000+/月 | Deep Think（高度な推論モード）含む |

## ChatGPT・Claudeとの使い分け

2026年のベストプラクティスは「モデルルーティング」—タスクに応じて最適モデルを選ぶことです。
- **Gemini 3.1 Pro**: マルチモーダル処理（画像・動画・音声）、Googleサービス連携（Gmail・Docs・YouTube）、長文脈処理
- **ChatGPT（GPT-5.2）**: エコシステムの成熟度、プラグイン・GPTs、クリエイティブライティング
- **Claude（Opus 4.6）**: コーディング、長文書分析、安全性重視のタスク`,
    relatedSlugs: ["gemini", "llm", "multimodal", "reasoning-model", "chatgpt", "claude"],
    sources: [
      {
        title: "Gemini 3 - Google Blog",
...
LangChain・LlamaIndex・AutoGen・CrewAIなどのエージェントフレームワークはすべてこのエージェントループを実装の核に置いており、現代のAIエージェント開発の基本概念です。`,
    relatedSlugs: ["agent", "agentic-workflow", "tool-use", "reasoning-model", "autonomous-agent", "react-agent"],
    sources: [
      {
        title: "ReAct: Synergizing Reasoning and Acting in Language Models",
        url: "https://arxiv.org/abs/2210.03629",
  ...
  },
  {
    ...
    description: `ReActエージェント（ReAct Agent）とは、Yao et al.（2022）が提案したReActフレームワーク（Reasoning + Acting）を実装したAIエージェントです。

ReActの核心は「思考と行動の交互実行」にあります。従来の行動のみのエージェントと異なり、各ステップで「なぜその行動を取るか」という推論を言語化することで、より透明性が高く、エラー訂正が容易なエージェントが実現します。

具体的なサイクルは以下の通りです：
Thought（思考）：現在の状態・目標・利用可能なツールを考慮して次の行動を決定します。
Action（行動）：search("keyword")やlookup(URL)などのツール呼び出しを実行します。
Observation（観察）：ツール実行結果を受け取り、次のThoughtに活かします。
...
ReActと対比されるPlan-and-Executeアーキテクチャでは、最初に全ステップを計画してから実行しますが、ReActは実行しながら計画を修正する点が特徴です。`,
    relatedSlugs: ["agent-loop", "agentic-workflow", "tool-use", "agent", "cot"],
    sources: [
      {
        title: "ReAct: Synergizing Reasoning and Acting in Language Models",
        url: "https://arxiv.org/abs/2210.03629",
      ...
      {
        title: "ReAct Agent - LangChain Docs",
        url: "https://python.langchain.com/docs/how_to/react/",
        publisher: "LangChain",
        accessedAt: "2026-03-24",
      },
    ],
  ...
  },
  {
    ...
    term: "マルチエージェントシステム",
    reading: "マルチエージェントシステム",
    category: "実装",
    summary:
      "マルチエージェントシステムとは、複数のAIエージェントが役割分担・協調しながら動作するアーキテクチャで、オーケストレーターとワーカーの階層構造で複雑なタスクを並列処理します。",
    description: `マルチエージェントシステム（Multi-Agent Systems）とは、複数のAIエージェントが相互に通信・協調しながら共同でタスクを解決するアーキテクチャです。単一エージェントでは処理しきれない複雑なワークフローや、長いコンテキストが必要な作業を分割・並列化するために用いられます。
...
主要なフレームワーク：
LangGraph（LangChain）はStateGraphという概念でエージェント間の状態遷移と通信を管理し、有向グラフでワークフローを定義します。複雑な条件分岐や並列実行を宣言的に記述できます。
AutoGen（Microsoft）はエージェント間の会話駆動型協調を特徴とし、UserProxy・AssistantAgent・GroupChatManagerなどのロールを組み合わせてマルチエージェント会話を構成します。
CrewAIはエージェントに「役割・目標・バックストーリー」を定義するRolePlayingフレームワークで、チームとしての動作を直感的に定義できます。

設計上の重要な考慮点：
エージェント間通信の設計：共有メモリ・メッセージキュー・直接API呼び出しによる通信方式の選択が性能と信頼性に影響します。
コンテキスト管理：各エージェントが保持するコンテキストの範囲を適切に制限し、全体コストを管理することが必要です。
エラー伝播の制御：あるエージェントの失敗が連鎖しないよう、フォールバックと再試行の設計が重要です。
Human-in-the-Loop：重要な判断ポイントでの人間介入をワークフローに組み込むことで安全性が高まります。

マルチエージェントシステムはソフトウェア開発（コーディング・テスト・レビューを別エージェントが担当）、データ分析（収集・前処理・可視化を並列処理）、顧客サポート（一次対応・専門対応・エスカレーションの階層化）などで実用化が進んでいます。`,
    relatedSlugs: ["agent", "agent-loop", "react-agent", "agentic-workflow"],
    sources: [
      {
  ...
  },
  {
    ...
    description: `ツールユースパターン（Tool Use Pattern）とは、大規模言語モデル（LLM）が外部のAPIやコード実行環境・データベースなどを「ツール」として呼び出す設計パターンです。LLM単体では持てないリアルタイム情報・計算能力・データ操作の能力をツール呼び出しによって補完し、エージェントとして実世界の操作を実現します。

主要な実装方式：
Function Calling（OpenAI）はGPT-3.5/4以降に導入された機能で、JSON Schemaで定義した関数をLLMに提示し、LLMが呼び出すべき関数名と引数をJSON形式で出力します。アプリケーション側でその関数を実際に実行して結果をLLMに返すことで、対話的なツール活用が実現します。
Anthropic Tool Use（Claude）はClaudeのAPIでtoolsパラメータに定義を渡すと、Claudeが必要に応じてtool_useコンテンツブロックを生成します。Anthropicのドキュメントでは、ツールの目的・入力スキーマ・使用例を明確に記述することで精度が向上するとされています。
MCP（Model Context Protocol）はAnthropicが提案したオープン標準プロトコルで、ツール定義とツール実行サーバーを統一的なインターフェースで繋ぎ、LLMアプリケーション間でのツール共有を可能にします。
...
ツール設計のベストプラクティス：
単一責任の原則：各ツールは明確に定義された1つの機能のみを持つことでLLMの選択精度が向上します。
詳細な説明文：ツールのdescriptionに「いつ使うべきか・使うべきでないか」を記述することで誤用を防ぎます。
構造化された入力スキーマ：required/optionalパラメータを明確にし、enumで値の範囲を制限することでLLMが正しい引数を生成しやすくなります。
エラーハンドリング：ツール実行失敗時にエラーメッセージをLLMに返し、代替手段を検討させるフローが重要です。
冪等性の確保：同じツールを複数回呼び出しても副作用が最小限になるよう設計することで、エージェントループの安定性が高まります。`,
    relatedSlugs: ["tool-use", "agent-loop", "react-agent", "mcp"],
    sources: [
      {
      ...
      {
        title: "Function calling - OpenAI Platform",
        url: "https://platform.openai.com/docs/guides/function-calling",
        publisher: "OpenAI",
        accessedAt: "2026-03-24",
      },
...
重要な注意点として、アテンション重みが高いからといって必ずしもそのトークンが予測に重要というわけではなく（「Attention is not Explanation」論争）、アテンションマップはあくまでモデル内部の一側面に過ぎません。機械論的解釈可能性（Mechanistic Interpretability）研究では、アテンションパターンより回路（circuit）レベルの分析が重要視される傾向があります。`,
  ...
  },
  {
    ...
    description: `クロスアテンション（Cross-Attention）とは、トランスフォーマーアーキテクチャにおけるアテンション機構の一形態で、クエリ（Q）をデコーダー側から、キー（K）とバリュー（V）をエンコーダー側から取得することで、2つの異なる系列間の関係を計算します。
...
Stable DiffusionやDALL-Eのようなテキスト条件付き画像生成モデルは、クロスアテンションなしには成立しません。マルチモーダルAIの理解に不可欠な基礎概念です。`,
    relatedSlugs: ["attention-mechanism", "encoder-decoder", "transformer", "multi-head-attention", "vision-language-model"],
    sources: [
      {
        title: "Attention Is All You Need",
        url: "https://arxiv.org/abs/1706.03762",
      ...
      },
      {
        ...
        publisher: "arXiv / Bahdanau et al.",
        accessedAt: "2026-02-26",
      },
    ],
    updatedAt: "2026-02-26",
  },
  {
    ...
    description: `アダプターチューニング（Adapter Tuning）とは、事前学習済みトランスフォーマーの各層に小さなボトルネック構造のモジュール（アダプター）を挿入し、元のモデルパラメータを完全に固定したまま新しいタスクに特化させるファインチューニング手法です。
...
実用面では、Hugging Face PEFTライブラリがアダプターチューニングをサポートしており、医療・法律・製造など特定ドメインへのLLM適応に利用されています。フルファインチューニングと比べストレージコストと計算コストを大幅に削減できます。`,
    relatedSlugs: ["peft", "lora", "prefix-tuning", "fine-tuning", "parameter"],
    sources: [
      {
        title: "Parameter-Efficient Transfer Learning for NLP",
        url: "https://arxiv.org/abs/1902.00751",
        publisher: "arXiv / Houlsby et al.",
        accessedAt: "2026-02-26",
      },
      {
  ...
  },
  {
    ...
    term: "センテンストランスフォーマーズ",
    reading: "センテンストランスフォーマーズ",
    category: "実装",
    summary:
      "センテンストランスフォーマーズとは、文章・段落を固定長の高品質ベクトルに変換するPythonライブラリで、BERTを双方向エンコーダとして文単位で学習させたSBERTが基盤となり、意味検索・RAG・類似度計算に広く使われます。",
    description: `Sentence Transformers（センテンストランスフォーマーズ）とは、文・段落・文書を意味的に近いものが近い距離に配置されるような密ベクトル（Dense Vector）に変換するPythonライブラリです。Nils Reimers が開発し、sbert.net で公開されています。
...
OpenAIのtext-embedding-ada-002やCohere Embedも同種の用途ですが、Sentence TransformersはオープンソースでローカルGPU上でも動作するため、コストとプライバシーを重視するユースケースに適しています。`,
    relatedSlugs: ["embedding", "semantic-similarity", "bert", "semantic-search", "hugging-face"],
    sources: [
      {
        title: "Sentence-BERT: Sentence Embeddings using Siamese BERT-Networks",
        url: "https://arxiv.org/abs/1908.10084",
        publisher: "arXiv / Reimers & Gurevych",
        accessedAt: "2026-02-26",
      },
      {
  ...
  },
  {
    ...
    description: `AI法律（AI Legal）とは、法律実務や法的サービスにおけるAI技術の活用を指します。弁護士・法務部門・コンプライアンス部門の業務効率化から、一般市民向けのリーガルアクセス向上まで幅広い応用があります。

主要なユースケースとして、契約書レビュー（リスク条項の自動検出・比較）、判例・法令の検索と要約、デューデリジェンス（M&A時の法的調査）、コンプライアンスモニタリング、訴状・法的文書のドラフト支援などがあります。

代表的なツールとして、HarveyはGPT-4をベースにした法律特化LLMで主要法律事務所が採用しています。CasetextはWestlaw等の法的データベースと統合したリサーチツールを提供しており、Thomson Reutersに買収されました。LexisNexis・Westlawも独自のAI機能を強化しています。

法律AIの課題として、ハルシネーション（架空の判例引用）のリスクが特に深刻です。法的アドバイスの責任所在、弁護士倫理規程との整合性、機密情報の取り扱いなど、他分野より高い精度・安全性基準が求められます。現状では弁護士の監督下でのアシスタントとしての活用が主流です。`,
    relatedSlugs: ["document-ai", "summarization", "information-retrieval", "ai-compliance", "ai-automation"],
    sources: [
      {
      ...
      {
        title: "Stanford CodeX - Center for Legal Informatics",
  ...
  },
  {
    ...
    description: `AI金融（AI Finance）とは、銀行・証券・保険・資産運用などの金融サービスにおけるAI技術の活用全般を指します。機械学習が先行して普及していた分野ですが、LLMの登場により自然言語処理を組み込んだ新たな応用が急拡大しています。

主要なユースケースは多岐にわたります。アルゴリズム・高頻度取引（HFT）での予測モデル、ローン審査・信用スコアリング、クレジットカード不正検出、KYC（顧客確認）・AML（マネーロンダリング対策）の自動化、ERPデータに基づく財務予測、個人向けロボアドバイザー、決算書・アナリストレポートの自動要約などがあります。

金融特化LLMとして、BloombergはBloomBERGPT（2023年）を開発し、FinGPTはオープンソースで金融データに特化したモデルを提供しています。JPMorgan・Goldman Sachsなど大手金融機関も独自のAI研究開発を進めています。

規制・コンプライアンスの観点から、金融AIは説明可能性（Explainability）が強く求められます。EU AIActや米国の公正貸出法（Fair Lending Laws）への対応、モデルバイアスの監査など、規制対応がAI活用の重要な課題となっています。`,
    relatedSlugs: ["predictive-analytics", "anomaly-detection", "ai-forecasting", "recommendation-system", "ai-automation"],
    sources: [
      {
        title: "BloombergGPT: A Large Language Model for Finance",
        url: "https://arxiv.org/abs/2303.17564",
        publisher: "arXiv / Bloomberg",
        accessedAt: "2026-02-26",
      },
      {
  ...
  },
  {
    ...
    description: `Mixture of Depths（MoD：深さの混合）とは、トランスフォーマーモデルの各層において、すべてのトークンを処理するのではなく、重要度に応じてトークンを選択的に処理するルーティング機構を導入した適応計算（Adaptive Computation）手法です。Google DeepMindのRaposoらが2024年に提案しました。
...
実験では、同じFLOPs予算でMoDを適用すると標準トランスフォーマーと同等以上の性能を達成しつつ、推論速度を改善できることが示されています。次世代の効率的なLLMアーキテクチャの有力候補として注目されています。`,
    relatedSlugs: ["mixture-of-experts", "transformer", "inference", "neural-architecture-search", "scaling-law"],
    sources: [
      {
        title: "Mixture of Depths: Dynamically allocating compute in transformers",
        url: "https://arxiv.org/abs/2404.02258",
        publisher: "arXiv / Raposo et al. (Google DeepMind)",
        accessedAt: "2026-02-26",
      },
    ],
    updatedAt: "2026-02-26",
  },
  {
    ...
    term: "画像キャプション生成",
    reading: "がぞうキャプションせいせい",
    category: "実装",
    summary:
      "画像キャプション生成とは、画像の内容を自然言語で説明するテキストを自動生成するタスクで、CLIPやViTなどのビジョンモデルとLLMを組み合わせ、アクセシビリティ・画像検索・医療画像説明に応用されます。",
    description: `画像キャプション生成（Image Captioning）とは、入力画像を解析し、その内容・シーン・物体・関係性を自然言語のテキストとして自動的に生成するコンピュータビジョンとNLPの融合タスクです。
...
評価指標にはBLEU・ROUGE・CIDErなどが使われますが、人間の評価との乖離が指摘されており、LLM-as-Judgeによる評価も普及しています。`,
    relatedSlugs: ["vision-language-model", "multimodal", "clip", "vision-transformer", "image-recognition"],
    sources: [
      {
        title: "Show, Attend and Tell: Neural Image Caption Generation with Visual Attention",
        url: "https://arxiv.org/abs/1502.03044",
        publisher: "arXiv / Xu et al.",
        accessedAt: "2026-02-26",
      },
      {
        title: "BLIP-2: Bootstrapping Language-Image Pre-training",
        url: "https://arxiv.org/abs/2301.12597",
        publisher: "arXiv / Salesforce Research",
        accessedAt: "2026-02-26",
      },
    ],
    updatedAt: "2026-02-26",
  },
  {
    ...
    term: "ビジュアルグラウンディング",
    reading: "ビジュアルグラウンディング",
    category: "実装",
    summary:
      "ビジュアルグラウンディングとは、自然言語の表現が指す画像内の領域・物体を特定するタスクで、「赤い車の左にある木」のような参照表現から画像内の対応領域を見つけ、マルチモーダルモデルとロボティクスで重要です。",
    description: `ビジュアルグラウンディング（Visual Grounding）とは、自然言語の表現（参照表現）と画像を入力とし、その言語表現が指し示す画像内の領域をバウンディングボックスやセグメンテーションマスクで特定するタスクです。
...
応用分野はロボットの物体操作（「赤いボールを取って」という指示への対応）、医療画像での病変部位の言語的参照、インタラクティブな画像編集（「空の部分を青く塗り替えて」）、マルチモーダルエージェントの視覚的推論などです。大規模VLMのグラウンディング能力評価は現在も活発な研究テーマです。`,
    relatedSlugs: ["vision-language-model", "grounding", "object-detection", "multimodal", "image-recognition"],
    sources: [
      {
  ...
  },
  {
    ...
    description: `AI創薬（AI Drug Discovery）とは、新薬の研究開発プロセスにAI・機械学習・深層学習を活用し、従来10〜15年・数千億円かかっていた開発期間とコストを短縮しようとする研究・産業分野です。

主要な応用領域は創薬の各段階に及びます。ターゲット発見では、ゲノム・プロテオーム・文献データから疾患関連タンパク質を特定します。構造予測では、DeepMindのAlphaFold2・3がほぼすべての既知タンパク質の3D構造予測を可能にし、創薬研究に革命をもたらしました。ヒット化合物発見では生成AIが新規分子構造を提案し、分子特性（毒性・吸収性・安定性）を予測します。臨床試験では患者層別化・試験設計最適化・バイオマーカー探索にAIが活用されます。

先端企業として、Insilico MedicineはAIが設計した候補化合物を臨床試験に進めた先駆者です。ExscientiaはAIデザインの分子を複数の疾患領域で臨床試験化しています。Isomorphic Labs（DeepMind系）はAlphaFoldを創薬に応用する研究を進めています。

課題として、AI予測の実験検証コスト、生物学的複雑性（細胞・個体レベルの効果）の予測の難しさ、規制当局（FDA・PMDA）のAI創薬ガイドラインの整備途上などがあります。`,
    relatedSlugs: ["alphafold", "ai-for-science", "deep-learning", "generative-model", "foundation-model"],
    sources: [
      {
        title: "Highly accurate protein structure prediction with AlphaFold",
        url: "https://www.nature.com/articles/s41586-021-03819-2",
      ...
      {
        title: "Insilico Medicine - AI Drug Discovery",
  ...
  },
  {
    ...
    term: "インストラクションデータセット",
    reading: "インストラクションデータセット",
    category: "実装",
    summary:
      "インストラクションデータセットとは、LLMの指示追従能力を学習させるための指示文と期待される回答のペアからなるデータセットで、Alpaca・FLAN・Dolly・ShareGPTが代表例であり、SFTとRLHFの品質を大きく左右します。",
    description: `インストラクションデータセット（Instruction Dataset）とは、大規模言語モデルに「指示を理解して適切に従う」能力を学習させるための訓練データです。「指示文（Instruction）」と「期待される回答（Response）」のペアで構成され、場合によってはコンテキスト（Input）も含みます。
...
データセット作成手法には、人手収集、Self-Instruct（LLMによる自動生成）、蒸留（強力なLLMの出力を教師として収集）、合成データ生成などがあります。指示のカバレッジ・多様性・難易度バランスが重要な設計要素です。`,
    relatedSlugs: ["instruction-tuning", "sft", "data-annotation", "synthetic-data", "dataset"],
    sources: [
      {
