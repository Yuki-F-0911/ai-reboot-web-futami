...
const subsidyGuideDescription =
  "リスキリング補助金を個人で活用するための対象条件、申請手順、AI研修での使い方を解説。受講料330,000円の計算例と最大70%補助のポイントを確認し、まずは無料説明会へ。";
...
const subsidyGuideFaqItems = [
  {
    ...
    answer:
      "経済産業省『リスキリングを通じたキャリアアップ支援事業』で、在職者が転職を目的に学ぶ際の費用を支援する制度です。受講修了で50%、転職後1年継続就業で追加20%、合計最大70%（上限56万円）が補助されます（2026年2月時点）。",
  },
  {
  ...
  },
  {
    ...
    answer:
      "公式情報では、受講修了で受講費用の50%（上限40万円）、転職後1年継続就業で追加20%（上限16万円）、合計最大70%（上限56万円）です。AIリブートアカデミー受講料¥330,000（税込、税抜¥300,000）の場合、税抜基準の計算例では最終的な実質負担は¥120,000です。",
  },
  {
  ...
  },
  {
    ...
    answer:
      "受講料は¥330,000（税込）です。税抜¥300,000を基準にした公式補助率の計算例では、受講修了時点の実質負担は¥180,000、追加補助まで達成した場合は¥120,000が目安です。",
  },
] as const;
...
const subsidyGuideHowToSteps = [
  ...
  {
    name: "受講修了で一次補助を適用",
    text: "受講料の50%相当を補助対象として処理します。",
  },
  {
    name: "転職後1年継続就業で追加補助",
    text: "追加20%の補助適用条件を満たすと、合計最大70%まで補助されます。",
  },
] as const;
