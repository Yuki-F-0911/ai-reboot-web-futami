...
const faqItems = [
  ...
  },
  {
    ...
    answer:
      "基本はPython 3.11〜3.13あたりが無難です（主要ライブラリの対応が揃いやすい）。ただしTensorFlowを使う予定がある場合は公式の対応状況を確認し、迷うなら3.12を選ぶとトラブルが少ないです。",
  },
  {
    question: "Anacondaと通常のPython（venv）はどちらがよいですか？",
    answer:
      "学習目的ならどちらでもOKです。データ分析系をまとめて入れたいならAnaconda、軽量にプロジェクト単位で管理したいならvenv（+ pip）がおすすめです。なお会社で使う場合、Anacondaのdefault channelは利用条件（TOS）の対象になるため注意。組織ルールが厳しいなら、venv運用か、conda系ならチャンネル設計まで含めて検討すると安全です。迷う場合はColabで学習を進め、必要になってからローカルへ移行するとスムーズです。",
  },
  {
