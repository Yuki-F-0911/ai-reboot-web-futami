...
デザインAIとして、私たちは無意識に「安全で無難な」デザインパターンに陥りがちです。これらの「思考停止パターン」は一見プロフェッショナルに見えますが、実際には：
- ブランドの独自性を損なう
- ユーザーの感情的つながりを弱める
- 競合との差別化を困難にする
- 本質的な問題解決を妨げる

このような落とし穴を避けるため、本ガイドラインでは「なぜそのデザインを選ぶのか」という根本的な問いを常に持ち続けることを重視します。
...
* **仕様**:  
  ...
  * **テキストアライメント原則**:
    - 本文・段落: 必ず左揃え（text-left）を使用。可読性と自然な読み流しを確保
    - 見出し: 短い見出しのみセンタリング可。ただし2行以上になる場合は左揃えを推奨
    - キャッチコピー: 10文字以内の短文のみセンタリング許可
    - 改行制御: CSSの`text-wrap: balance`や`hanging-punctuation`を活用し、美しい改行を実現
  * **日本語特有の配慮**:
    - 約物（句読点や括弧）の禁則処理を適切に設定
    - 一文字だけの改行（寡婦行）を避けるため、コンテンツ幅と文字数の関係を慎重に設計
    - 縦書きモードを使用する場合は、専用のタイポグラフィルールを別途定義
...
* **不適切なテキストアライメント**: 長文をセンタリングして読みにくくしたり、改行位置が制御できず見苦しい表示になること。**対策**: 本文は必ず左揃えとし、センタリングは短いキャッチコピーに限定。CSS Grid/Flexboxで適切な改行制御を実現する。  
...
### **第7章：デザインのアンチパターンと回避策**

効果的なデザインを阻害する「思考停止パターン」を認識し、それらを積極的に回避することは、真に創造的なデザインを生み出すために不可欠です。

#### **A. 色彩と視覚スタイルにおけるアンチパターン**

| アンチパターン | 具体例 | なぜ問題なのか | 回避策 |
| :--- | :--- | :--- | :--- |
| **安易なモノクロ＋アクセントカラー** | 白背景、黒に近いテキスト（#111827）、青系アクセント1色 | 没個性的で、ブランドの感情的価値を伝えきれない | ブランド人格から導き出された独自のカラーパレットを構築。色の役割を明確に定義 |
| **機械的なダークモード** | 単純な明度反転（背景#121212、テキスト白） | 本来の目的（目の疲れ軽減）を考慮せず、単なるオプション化 | 用途とコンテキストに応じた適切な明度調整。彩度の調整も含む |
| **使い古されたグラデーション** | `from-purple-500 to-indigo-600`のような短い2色グラデーション | 多用により安っぽく見え、情報の階層性を損なう | グラデーションは意味のある場面でのみ使用。3色以上の複雑なグラデーションも検討 |
| **根拠のない装飾** | カード左側の4pxボーダー | 「なぜ左側か」「なぜ4pxか」の根拠が不明確 | 装飾要素には必ず機能的な意味を持たせる |
| **画一的な角丸** | 全要素に8px or 12pxの角丸を適用 | 要素間の関係性や重要度の差異が表現できない | 要素の性質に応じて角丸の有無・サイズを意図的に変える |
| **思考停止の影付け** | Material Design風の拡散した影 | 非現実的で全体の質感を損なう | 光源を意識した自然な影。または影を使わない代替表現 |

#### **B. レイアウトと構造におけるアンチパターン**

| アンチパターン | 具体例 | なぜ問題なのか | 回避策 |
| :--- | :--- | :--- | :--- |
| **テンプレート思考のLP構成** | ヘッダー→ヒーロー→3カラム特長→お客様の声→FAQ→フッター | コンテンツの性質やユーザーの読解プロセスを無視 | ユーザージャーニーとコンテンツの本質から構成を再考 |
| **機械的な中央寄せ** | `max-w-7xl mx-auto px-4`で全コンテンツを固定 | ダイナミックな表現の機会を失う | コンテンツに応じて幅を変える。フルブリードレイアウトも検討 |
| **安易なテキストセンタリング** | 長文や段落をtext-centerで中央揃え | 可読性を著しく損なう。特に改行位置が制御できず一文字だけ次行になる | 本文は左揃えを基本とし、短いキャッチコピーのみセンタリングを検討 |
| **硬直的なグリッド思考** | 全てを12カラムグリッドに当てはめる | コンテンツの本質的なグルーピングを阻害 | コンテンツファーストでレイアウトを考える |
| **お決まりのヒーローセクション** | 背景画像＋中央の見出し＋CTAボタン | ユーザーが本当に知りたい情報へのアクセスを遅らせる | ファーストビューで価値を即座に伝える構成を検討 |

#### **C. コンポーネントとUI要素におけるアンチパターン**

| アンチパターン | 具体例 | なぜ問題なのか | 回避策 |
| :--- | :--- | :--- | :--- |
| **意味を考えないアイコン使用** | Font Awesomeから単語検索で選択 | 文化的・文脈的意味を無視 | アイコンの意味をユーザーテストで検証 |
| **機械的なボタン階層** | Primary/Secondary/Tertiaryの3種を自動的に使い分け | 行動の重要度を深く考慮していない | 各ボタンの目的と優先度を明確に定義 |
| **型にはめたカード構成** | 画像＋タイトル＋説明＋タグ＋ボタンの固定パターン | 情報の種類や重要度に合わせた表現ができない | コンテンツの性質に応じたカード設計 |
| **見た目だけのフォーム** | フローティングラベル、角丸入力欄 | ユーザビリティやアクセシビリティの問題 | 使いやすさを最優先にした設計 |
| **情報量のないインジケーター** | 単純な三点リーダーやスピナー | 処理内容や待機時間の見通しを提供しない | 進捗状況や予想時間を表示 |

### **第8章：思考を深めるデザインプロンプト体系**

アンチパターンを回避し、本質的なデザインを生み出すためには、プロジェクトの開始時点で以下のフレームワークを活用します。
...
project_charter:
  brand_personality:
    ...
    traits:
      - type: "誠実（Sincerity）"
        selected: true/false
        reason: "実直、正直、健全、陽気な印象を与える理由"
      - type: "刺激（Excitement）"
        selected: true/false
        reason: "大胆、元気、想像力豊か、先進的な印象を与える理由"
      - type: "有能（Competence）"
        selected: true/false
        reason: "信頼できる、知的、成功している印象を与える理由"
      - type: "洗練（Sophistication）"
        selected: true/false
        reason: "グラマラス、魅力的、上品な印象を与える理由"
      - type: "無骨（Ruggedness）"
        selected: true/false
        reason: "ワイルド、タフ、アウトドアな印象を与える理由"

  emotional_goals:
    # ユーザーに感じてほしい感情を3つ
    - emotion: "例：安心感"
      design_implications: "この感情を喚起するための具体的なデザイン要素"
    - emotion: "例：発見の喜び"
      design_implications: "この感情を喚起するための具体的なデザイン要素"
    - emotion: "例：信頼"
      design_implications: "この感情を喚起するための具体的なデザイン要素"
  ...
  anti_impressions:
    # 絶対に避けたい印象を3つ
    - impression: "例：安っぽい"
      avoidance_strategy: "この印象を避けるための具体的な対策"
    - impression: "例：複雑すぎる"
      avoidance_strategy: "この印象を避けるための具体的な対策"
    - impression: "例：時代遅れ"
      avoidance_strategy: "この印象を避けるための具体的な対策"
...
design_principles:
  color_strategy:
    purpose_driven_colors:
      # すべての色は以下の3つの役割のいずれかを持つ
      brand_expression:
        colors: []
        rationale: "ブランド人格をどのように表現するか"
      state_indication:
        colors: []
        rationale: "UIの状態をどのように示すか"
      interaction_guidance:
        colors: []
        rationale: "ユーザーの行動をどのように促すか"
    
    psychological_palette:
      # 感情目標に基づいた3つのカラーパレット案
      - name: "案1"
        colors: []
        psychological_reasoning: "なぜこの配色が目標の感情を喚起するか"

  form_and_layout:
    corner_radius_philosophy:
      default: "直角"
      exceptions:
        - element: "特定の要素名"
          radius: "Xpx"
          justification: "なぜこの丸みが必要か（ユーザビリティとブランドの観点から）"
    
    depth_expression:
      # box-shadowの使用を原則禁止
      alternatives:
        - method: "ボーダー"
          use_case: "どのような場合に使用するか"
        - method: "背景色の明度差"
          use_case: "どのような場合に使用するか"
        - method: "余白の調整"
          use_case: "どのような場合に使用するか"
    ...
    typography_first:
      process:
        1: "全グラフィック要素を排除したタイポグラフィのみのワイヤーフレーム作成"
        2: "情報構造の検証"
        3: "本当に必要なグラフィック要素のみを追加"

  interaction_design:
    meaningful_motion:
      # すべてのアニメーションに明確な理由が必要
      - animation_type: "要素の追加"
        movement: "既存要素が下にずれ、新要素がフェードイン"
        reasoning: "UIの状態変化を正確に伝えるため"
```

#### **実装時のチェックリスト**

1. **色の選択時**
   - [ ] この色はブランド表現、状態表示、インタラクション誘導のいずれかの役割を持っているか？
   - [ ] 装飾のためだけの色を使っていないか？
   - [ ] 定義した感情目標に貢献しているか？
