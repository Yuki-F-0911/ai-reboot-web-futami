...
# ⚠️ export $(grep ...) は .env.local 内の URL（https://...）でシェルエラーになる
# 必ず下記の形式で読み込む
set -a && source "$PROJECT_PATH/.env.local" && set +a

# API キーの存在確認
echo "XAI_API_KEY: ${XAI_API_KEY:0:8}..."
echo "X_BEARER_TOKEN: ${X_BEARER_TOKEN:0:8}..."
...
**Step 1-A: 高エンゲージメント絞り込みクエリで取得**

`lang:ja` は不完全（Thai/Indonesianが混入する）。代わりに `min_faves:20` でスパム・低品質投稿を自然に除外する。
件数は 50 で十分（100はノイズが増えるだけ）。

```bash
curl -s "https://api.twitter.com/2/tweets/search/recent" \
  -H "Authorization: Bearer $X_BEARER_TOKEN" \
  -G \
  --data-urlencode 'query=生成AI OR ChatGPT OR Claude OR Gemini OR 大規模言語モデル -is:retweet lang:ja min_faves:20' \
...
### Phase 2: Grok API で分析 → WebSearch ファクトチェック
...
curl -s https://api.x.ai/v1/chat/completions \
  ...
  -d '{
    ...
    "messages": [
      {
        "role": "system",
        "content": "あなたは生成AIの動向を初学者・ビジネスパーソン向けにわかりやすく解説するエディターです。"
      },
      {
        "role": "user",
        "content": "今日は{YYYY-MM-DD}です。直近7日間の生成AIに関する重要ニュースを3〜5個選び、重要度・信頼性・ビジネス影響を評価してください。X投稿データ: {X_POSTS_DATA}\n\n出力: JSON形式で { topics: [{ title, summary, importance, sources: [{ name, url }] }] }"
      }
    ]
  }'
```

**【必須】WebSearch ファクトチェック:**

Grok API が返す `sources[].url` は幻覚（存在しないURL）の場合がある。各トピックについて以下を実行する:

```
1. WebSearch で「{トピックタイトル} {YYYY}年{M}月」を検索
2. 実在する記事・公式発表が見つかれば採用
3. 見つからない・内容が異なる場合:
   - トピックを差し替えるか
   - 出典URLを正しいものに差し替えるか
...
Grok API が使えない場合は Claude（自分）が WebSearch のみでリサーチ・分析を代替する。
...
| 項目 | 内容 |
|---|---|
| X API 制限 | 無料プランは直近7日・月500リクエスト。max_results=50 + min_faves:20 で節約。コスト発生後は全スキップ禁止、必ずフィルタして活用 |
