...
const fiveSteps = [
  ...
  },
  {
    ...
    body: "メール/カレンダー/CRM/社内DBなど、実務のボトルネックに直結するツールから選びます。最初は「読み取り中心」→「限定的な書き込み」→「承認付き実行」の順に段階的に権限を広げると安全です。ツール連携は、MCP（Model Context Protocol）などの共通プロトコル経由で接続できる場合もあります。",
  },
  {
...
const frameworkRows = [
  ...
  },
  {
    ...
    cautions:
      "要件次第で実装の自由度（状態/評価/ツール制御）に制約が出る。Assistants APIはdeprecated（2026年2月時点で削除予定: 2026年8月）なので、新規はResponses API前提で検討する",
  },
] as const;
